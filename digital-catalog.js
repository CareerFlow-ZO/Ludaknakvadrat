(() => {
  ...
  .dc-monthly{margin-top:65px}.dc-monthly h3{font-family:Cinzel,serif;font-size:34px;margin-bottom:8px}.dc-monthly>p{color:#918c82}.dc-monthly-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:24px}.dc-monthly-card{padding:26px;border:1px solid #3a2d17;border-radius:18px;background:#0d0c09}.dc-monthly-visual{position:relative;height:145px;margin:-26px -26px 17px;border:0;border-bottom:1px solid rgba(95,185,255,.13);padding:0;background:#07101a;overflow:hidden;display:block;cursor:pointer}.dc-monthly-visual img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .25s}.dc-monthly-card:hover .dc-monthly-visual img{transform:scale(1.025)}.dc-monthly-card h4{color:#e2b34b;font-size:18px}.dc-monthly-card .dc-price{font-size:26px}.dc-monthly-card p{color:#918c82;min-height:70px}
  ...
  function render(){
    ...
    section.innerHTML=`<div class="dc-wrap"><div class="dc-head"><span class="eyebrow">${t.eyebrow}</span><h2>${t.title}</h2><p>${t.intro}</p></div>
      ...
      <div class="dc-monthly"><h3>${t.monthly}</h3><p>${t.monthlyText}</p><div class="dc-monthly-grid">${monthly.map(m=>{const img=previewSrc(m.name,'tech','✦');return `<div class="dc-monthly-card"><button type="button" class="dc-monthly-visual" data-lnk-preview-src="${img}" data-lnk-preview-title="${m.name}"><img src="${img}" alt="${m.name} — LNK DIGITAL preview" loading="lazy"></button><h4>${m.name}</h4><div class="dc-price">${t.from} ${m.price}</div><p>${m.text}</p><a class="lnk-order-btn dc-month-order" href="#digital-order" data-service="${m.name} — ${m.price}">${t.order}</a></div>`}).join('')}</div></div>
