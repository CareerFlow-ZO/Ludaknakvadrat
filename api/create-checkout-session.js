  ...
  try {
    ...
    const orderRows = await supabase("orders", {
      ...
      body: JSON.stringify({
        ...
        style: clean(style, 120),
        language: clean(language, 60),
        package: packageKey,
        amount_cents: amountCents,
        currency: "eur",
        mood: clean(mood, 80),
        deadline: clean(deadline, 80),
        story: clean(story, 1800),
        status: "pending"
      })
    });
    ...
    if (!order?.id) {
      throw new Error("Narudžbina nije sačuvana.");
    }
    ...
    return res.status(500).json({
      error:
        "Plaćanje trenutno nije dostupno. Proveri podešavanja ili naruči preko WhatsApp-a."
    });
  }
};
